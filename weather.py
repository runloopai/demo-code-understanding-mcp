...
generate_repo_map_cmd = f"cd /home/user/runloop-examples && wget -qO- https://aider.chat/install.sh | sh && touch ./generated_repo_map.txt && aider --model o3-mini --api-key openai={oaikey} --yes-always --show-repo-map > ./generated_repo_map.txt"
...
@mcp.tool()
async def run_kit_cli_get_file_tree(github_link: str, command: str):
    """
    Run the kit cli with the file-tree argument to return a json object of the file tree.
    """
    devbox_id = await launch_devbox_with_code_mount(github_link)
    result = runloop_client.devboxes.execute_sync(devbox_id, command="cd /home/user/runloop-examples && python /home/user/kit_cli.py file-tree")
    return json.dumps(result.model_dump())

@mcp.tool()
async def run_kit_cli_extract_symbols(github_link: str, command: str):
    """
    Run the kit cli with the extract-symbols argument to return a json object of the symbols.
    """
    devbox_id = await launch_devbox_with_code_mount(github_link)
    result = runloop_client.devboxes.execute_sync(devbox_id, command="cd /home/user/runloop-examples && python /home/user/kit_cli.py extract-symbols")
    return json.dumps(result.model_dump())
